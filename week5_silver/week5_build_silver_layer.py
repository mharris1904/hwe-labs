...
bronze_reviews = spark.readStream \
    ...
    .load(f"s3a://hwe-fall-2023/{handle}/bronze/reviews")
bronze_reviews.createOrReplaceTempView("bronze_reviews")

bronze_customers = spark.read.format("parquet").load(f"s3a://hwe-fall-2023/{handle}/bronze/customers")
...
streaming_query = silver_data.writeStream \
    .format("parquet") \
    .option("path", f"s3a://hwe-fall-2023/{handle}/silver/reviews/") \
